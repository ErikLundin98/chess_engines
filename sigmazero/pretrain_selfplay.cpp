...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		for(int worker_idx = 0 ; worker_idx < batch_size ; ++worker_idx)
		{
			...
			chess::position state = worker.get_position();
			double value = eval::evaluate_material_normalized(state);
			
			torch::Tensor encoded_state = model->encode_input(state);
			worker.output_current(std::cout, encoded_state, value);
			worker.make_best_move(encoded_state, do_full_search);
			// std::cerr << eval::evaluate_material_normalized(chess::position::from_fen("rnbqkbnr/pppp1ppp/8/4P3/8/8/PPP1PPPP/RNBQKBNR b KQkq - 0 1")) << std::endl;
